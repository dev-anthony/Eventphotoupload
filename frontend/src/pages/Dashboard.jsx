...
export default function Dashboard() {
  ...
  const handleDeleteEvent = (eventId) => {
    // Show confirmation modal FIRST - ask user if they're sure
    setConfirmModal({
      isOpen: true,
      title: 'Delete Event?',
      message: 'Are you sure you want to delete this event? All photos will be permanently deleted. This action cannot be undone.',
      onConfirm: async () => {
        // This function only runs when user clicks "Delete" button
        try {
          const response = await eventApi.deleteEvent(eventId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400">
      ...
      {/* Alert Modal - Shows success/error messages */}
      <AlertModal
        isOpen={alertModal.isOpen}
        onClose={() => setAlertModal({ ...alertModal, isOpen: false })}
        type={alertModal.type}
      ...
      {/* Confirm Modal - Asks for confirmation before delete */}
      <ConfirmModal
        isOpen={confirmModal.isOpen}
        onClose={() => setConfirmModal({ ...confirmModal, isOpen: false })}
        onConfirm={confirmModal.onConfirm}
